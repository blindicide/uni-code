...
DATABASE = os.path.join(SCRIPT_DIR, 'student_database.db')
...
def get_db_connection():
    ...
    conn.row_factory = sqlite3.Row # Возвращать строки как словари (объекты, похожие на словари)
...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS Student_Subjects (
        ...
        FOREIGN KEY (subject_id) REFERENCES Subjects(id) ON DELETE CASCADE -- При удалении предмета удаляются и его связи с студентами
    )''')
